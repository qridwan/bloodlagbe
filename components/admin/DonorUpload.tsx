...
export default function AdminDonorUpload() {
  ...
  return (
    <div className="p-6 border rounded-xl shadow bg-white max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="w-full">
          <label
            ...
            onDrop={handleDrop}
            className={`w-full border-2 rounded-lg cursor-pointer text-center transition ${
              dragActive ? 'border-indigo-600 bg-indigo-50' : 'border-dashed border-gray-300'
            }`}
          >
            <input
              id="donor-file-input"
      ...
      {message && (
        <div
          className={`mt-4 p-4 rounded-md flex items-center gap-2 text-sm ${
            errorDetails ||
      ...
      {errorDetails && errorDetails.length > 0 && (
        <div className="mt-4">
          <h3 className="text-md font-semibold text-red-700">
            Error Details ({errorDetails.length} rows):
          </h3>
