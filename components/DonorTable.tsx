			...
			<table className="min-w-full divide-y divide-gray-200">
				<thead className="bg-gray-100">
					<tr>
						...
						>
							<div>
								Name <span className="text-gray-500 lowercase block text-xs">(tag)</span>
							</div>
						</th>
						<th
							scope="col"
            ...
            */}
				...
				<tbody className="bg-white divide-y divide-gray-200">
					{donors.map((donor) => (
						<tr
							key={donor.id}
							className={` ${donor.isAvailable ? 'cursor-pointer hover:bg-gray-50' : 'cursor-not-allowed opacity-80 bg-gray-200'}`}
						>
							<td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
								{donor.name}
								{donor.tagline && (
									<span className="text-gray-500 block text-xs">({donor.tagline})</span>
								)}
							</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm">
								<span
									className={`inline-block px-2 py-1 rounded-full font-semibold tracking-wide shadow-sm border ${bloodGroupColor(donor.bloodGroup)}`}
								>
									{formatBloodGroup(donor.bloodGroup)}
								</span>
							</td>
							<td className="px-6 py-4 h-full text-sm text-gray-700">
								<div className="flex gap-2 h-full justify-start items-center flex-wrap">
									<Telephone {...donor} />
								</div>
							</td>
							...
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{donor.city}</td>
