...
export default function ManageGroupsPage() {
  ...
  const openModal = (mode: 'add' | 'edit', group?: Group) => {
    // Changed type to Group
    setModalMode(mode);
    setFormError(null);
    if (mode === 'edit' && group) {
  ...
  if (isLoading && sessionStatus === 'authenticated' && !error) {
    return (
      <div className="text-center p-10">
        <p className="text-lg animate-pulse text-sky-600">Loading Groups...</p>
      </div>
    ); // Updated message
  }
  if (sessionStatus === 'authenticated' && session?.user?.role !== 'ADMIN') {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Manage Social Groups</h1>{' '}
          {/* Updated title */}
          <p className="text-slate-600 mt-1 text-sm">
            Add, edit, or delete social group names.
          </p>{' '}
          {/* Updated description */}
        </div>
        <button
      ...
      {!isLoading &&
        !error &&
        groups.length === 0 && ( // Changed state name
          <div className="text-center py-10 bg-white p-6 rounded-lg shadow-md">
            ...
            <p className="text-sm text-gray-400 mt-2">Click "Add New Group" to get started.</p>{' '}
            {/* Updated message */}
          </div>
        )}
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md space-y-4">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                <button type="button" className="border p-2" onClick={closeModal}>
                  {' '}
                  Cancel{' '}
                </button>
                <button
