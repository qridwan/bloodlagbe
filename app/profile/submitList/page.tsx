...
export default function SubmitListPage() {
  ...
  useEffect(() => {
    ...
    if (savedDraftString) {
      try {
        ...
        if (
          (reviseId && draft.associatedReviseId === reviseId) ||
          (!reviseId && !draft.associatedReviseId)
        ) {
          setListName(draft.listName || '');
          setListNotes(draft.listNotes || '');
          setParsedData(draft.parsedData || []);
          setHeaders(
            draft.headers && draft.headers.length > 0
              ? draft.headers
              : EXPECTED_CSV_HEADERS_SNAKE_CASE
          );
  ...
  useEffect(() => {
    if (sessionStatus === 'authenticated' && initialLoadAttempted.current) {
      // Only save after initial load attempt
      const draft = {
        listName,
        listNotes,
        parsedData,
      ...
      if (parsedData.length > 0 || listName.trim() !== '' || listNotes.trim() !== '') {
        ...
        // If it's a new submission and all fields are empty, clear storage
        sessionStorage.removeItem(SESSION_STORAGE_KEY);
      }
    }
    ...
    if (reviseId && sessionStatus === 'authenticated') {
      ...
      const fetchSubmissionForRevision = async () => {
        try {
          ...
          if (!response.ok) {
            throw new Error('Failed to load submission for revision.');
          }
          ...
          if (savedDraftString) {
            const draft = JSON.parse(savedDraftString);
            if (
              draft.associatedReviseId === reviseId &&
              draft.timestamp > new Date(data.submittedAt).getTime()
            ) {
              // Or a different timestamp if available
              ...
              console.log(
                'Revision data fetched from API. Session draft (if any for this ID) will be overwritten by new edits.'
              );
            }
          }
  ...
  const handleParseCsv = () => {
    ...
    Papa.parse(csvFile, {
      ...
      complete: (results) => {
        ...
        setHeaders(
          processedHeaders.length > 0 ? processedHeaders : EXPECTED_CSV_HEADERS_SNAKE_CASE
        );
        const missing = EXPECTED_CSV_HEADERS_SNAKE_CASE.filter(
          (eh) => !processedHeaders.includes(eh)
        );
        if (missing.length > 0 && processedHeaders.length > 0) {
          // only error if headers were actually parsed but are wrong
          setParseError(
            `Missing/mismatched headers: ${missing.join(', ')}. Ensure all expected headers are present and correctly named (e.g., 'blood_group').`
          );
          setParsedData([]);
        } else {
          const dataWithClientIds = (results.data as ParsedDonor[]).map((row, index) => ({
            id: `csv_${index}_${Date.now()}`, // Ensure unique key for React
            ...EXPECTED_CSV_HEADERS_SNAKE_CASE.reduce((obj, header) => {
        ...
        {!reviseId && (
          <div className="mt-6 border-t pt-4 text-sm text-gray-700">
            ...
            <p className="mb-3 text-gray-600">
              Please ensure your CSV file includes the following headers, in the exact order shown
              below:
            </p>
            ...
            <div className="bg-slate-50 border rounded p-4 font-mono overflow-auto text-xs md:text-sm leading-relaxed">
              ...
              <code>
                name, blood_group, contact_number, email, district, city, campus, group,
                is_available, tagline
              </code>

              <p className="mt-4 text-indigo-600 font-semibold">Example Row:</p>
              <code>
                Rahim Sheikh,O_POSITIVE,01711223344,rahim.s@example.com,Dhaka,Gulshan,University of
                Dhaka,Badhan,TRUE,CSE'2025
              </code>
            </div>

            <ul className="list-disc list-inside mt-4 space-y-1 text-gray-600">
              <li>
                <strong>name</strong>: Full name of the donor.
              </li>
              <li>
                <strong>blood_group</strong>: Valid blood type (e.g., A_POSITIVE, O_NEGATIVE).
              </li>
              <li>
                <strong>contact_number</strong>: Phone number (e.g., 017xxxxxxxx).
              </li>
              <li>
                <strong>email</strong>: Valid email address.(optional)
              </li>
              <li>
                <strong>district</strong>: District name (e.g., Dhaka, Chattogram).
              </li>
              <li>
                <strong>city</strong>: City or area (e.g., Gulshan, Uttara).
              </li>
              <li>
                <strong>campus</strong>: Institution or university campus name(optional).
              </li>
              <li>
                <strong>group</strong>: Name of donor group (e.g., Badhan, Medicine Club)(optional).
              </li>
              <li>
                <strong>is_available</strong>: TRUE or FALSE — donor's availability status.
              </li>
              <li>
                <strong>tagline</strong>: Additional info (e.g., 1st Batch, Dept. of CSE)(optional).
              </li>
            </ul>

            <p className="mt-4 text-sm text-gray-500">
              ✅ <span className="text-green-600 font-medium">Pro Tip:</span> You can use Google
              Sheets or Excel to create your list and export it as a <code>.csv</code> file.
            </p>
          </div>
        )}
      </header>
        ...
        {parsedData.length > 0 && (
          <section className="space-y-4 mt-6">
            ...
            <p className="text-sm text-gray-500">
              Click on a cell to edit its content. Press Enter or click outside the cell to save the
              change.
            </p>
                ...
                <thead className="bg-gray-100 sticky top-0 z-10">
                  <tr>
                    {headers.map((header) => (
                      <th
                        key={header}
                        className="px-3 py-2.5 text-left font-medium text-gray-600 uppercase tracking-wider"
                      >
                        {header.replace(/_/g, ' ')}
                      </th>
                    ))}
                    <th className="px-3 py-2.5 text-left font-medium text-gray-600 uppercase tracking-wider">
                      Actions
                    </th>
                  </tr>
                </thead>
